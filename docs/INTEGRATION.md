...
### 7.1 The alignment: Tailwind's rem utilities ARE finger units

This is the single most important property of running the two together, and it is by design —
Square Root was built to share Tailwind's ruler (rem), which is why Tailwind replaced Bootstrap in
its author's stack: Bootstrap's scale had to be removed, Tailwind's is the unit.

Tailwind's spacing and type scales are **in rem**. Square Root's unit is `--micro-width × 0.0625rem`
= **3.75rem**, and the solver changes only the **root font-size**. Therefore every Tailwind rem
utility on the page is a fixed fraction of a finger unit (`rem ÷ 3.75`: `p-3` = `0.75rem` = `0.2u`)
and scales with the device exactly as the `sqr-*` ones do — see the table in the README, *Built hand
in hand with Tailwind*. What follows is the same fact seen from the type side.
...
## 11. The Tailwind plugin

Since 0.5.0 the framework is also available as a Tailwind CSS plugin — the same
classes §7 shows you linking as a stylesheet, generated by Tailwind instead.

**Tailwind v3.3+** (`tailwind.config.js`):
...
What changes versus linking `dist/square-root.css`:

- **Arbitrary values.** `sqr-w-[1/2]`, `sqr-mt-[1.75]`, `sqr-px-[0.25]` — any
  number of finger units, generated on demand by JIT. The static stylesheet
  only has the 1–8 scale plus the few fractions listed in MEASURING.md; the
  plugin removes that limitation. The bracket value is the unit count, nothing
  else — `sqr-w-[30px]` is meaningless and will produce broken CSS.
- **IntelliSense** autocompletes `sqr-*` classes; unused utilities are purged.
- **Do not also link `dist/square-root.css`** — you would ship every rule
  twice. One source of the framework per page.

What does *not* change:

- **The solver is still required.** The plugin is the stylesheet half only;
  the style tag, the two probes and `src/square-root.js` are set up exactly as
  in §1. A Tailwind build cannot solve a viewport at runtime.
- **The scroll-snap layer is not in the plugin.** Link `dist/scrollsnap.css`
  if you use it.
- **The `2xs:`/`xs:` prefixed classes are not in the plugin** — their names
  collide with Tailwind's variant syntax. Use Tailwind's own variants:
  `max-[320px]:hidden` for the watch rules, `min-[321px]:sqr-mx-[0.25]` for
  the rest.

Parity between the plugin and the SCSS is enforced, not assumed:
`npm run verify:plugin` compiles the plugin through Tailwind and compares
every emitted rule against `dist/square-root.css`, failing on any real
difference. If you change one emitter, run it before you commit.
